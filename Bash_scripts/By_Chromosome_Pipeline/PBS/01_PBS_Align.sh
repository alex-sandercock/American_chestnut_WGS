...
####                                										###
####
#### Usage to submit 1 job per sample file: find `pwd` -maxdepth 1 -iname '*.1.fq.gz' -exec qsub 01_PBS_Align.sh -v FASTAFILE='{}' \;
###############################################################################
...
outdir="/work/newriver/amsand/Pure_American_Analysis/SAMs/"$sample".sam" #make sure you have SAMs created. mkdir SAMs

bwa mem -M -R "@RG\tID:Sample_"$sample"\tSM:"$sampleRGSM"" -t 20 /work/newriver/amsand/ChestnutPureGenome/Castanea_dentata.mainGenome.fasta \
